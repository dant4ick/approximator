function multiple_integrate_left_rect(
    f, dxStart, dxEnd, dxSplit, dyStart, dyEnd, dySplit, method) {
...
function multiple_integrate_right_rect(
    f, dxStart, dxEnd, dxSplit, dyStart, dyEnd, dySplit, method) {
  let dyStep = (dyEnd - dyStart) / dySplit;
...
function multiple_integrate_trap(
    f, dxStart, dxEnd, dxSplit, dyStart, dyEnd, dySplit, method) {
  let dyStep = (dyEnd - dyStart) / dySplit;
...
function multiple_integrate_par(
    f, dxStart, dxEnd, dxSplit, dyStart, dyEnd, dySplit, method) {
  let dyStep = (dyEnd - dyStart) / dySplit;
  ...
  const expression = math.parse(f).compile();
  return (expression.evaluate({'x': dyStart, 'y': dxStart}) +
      expression.evaluate({
        'x': dyEnd,
        'y': dxEnd,
